...
ggplot(cellsize_growth, aes(x=Fee, y=adjustedsize, color = Temperature))+
  stat_summary(fun.y = mean, geom = "point", size =4, aes(color = Temperature, group = Temperature))+
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 30, aes(color = Temperature))+
   scale_color_manual(values = c("black", "orange", "red", "darkred"))+
  theme_bw()+
  ylab(expression (Estimated ~Cell ~Diameter~(mu*m)))+
  xlab (expression (Fe*"'"~ (pMole~L^-1)))+  theme(axis.text.x=element_text(face = "bold", color = "black", size = 20),
        axis.title.x=element_text(size=24,face="bold"), 
        axis.text.y=element_text(face = "bold", size = 20, color = "black"),
...
#--------------------------------
ggplot(cellsize_growth, aes(x=totaliron_1, y=adjustedsize, color = Temperature, group = totaliron_1))+
  geom_violin(aes(colour = "black"))+
  ...
  theme(panel.grid = element_blank())+
   theme (legend.title = element_text (size = 20), legend.text =   element_text(size = 16))+
  labs(col="Temperature (°C)")+theme(axis.text.y = element_blank())+
  scale_y_discrete(breaks=FALSE)
  



  ggplot(cellsize_growth_3, aes(x = totaliron_1,
             y = adjustedsize))  +
  geom_point(group = Temperature) +
  theme_bw() 
