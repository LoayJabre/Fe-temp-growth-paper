...
stats for monod plots using nlme
```{r}
mergedata_sub$Temperature <- as.numeric(mergedata_sub$Temperature)
...
# 
# 
# mumax_nlme_1b <- nlme(mumax ~ SSmicmen(Fee, Vm, K), 
#                       fixed = list(Vm+K~as.factor(Temperature)), 
#                       random = Vm+K~1 | replicate, 
#                       start = list(fixed = c(Vm = c(0.1, 0.3, 0.4), K= c(25, 40, 60))),
#                       data = mergedata_sub)
...
anova(mumax_nlme_1, mumax_nlme_2, mumax_nlme_3)

summary(mumax_nlme_3)
#$tTable[,2]
...
#Kfe plot
#png("figure2_A_Kfe-vs-temp.png", width=45*0.75, height=23*0.75, units="cm", res=900)
ggplot (Km_temp, aes(x = Temperature, y = Kfe))+
  geom_point(size=6)+
  stat_function(fun = monod_fig2,size = 0.8 )+
...
#muplot
png("figure2_B_mu-vs-temp.png", width=45*0.75, height=23*0.75, units="cm", res=900)
ggplot (Km_temp, aes(x = Temperature, y = mu))+
  geom_point(size=6)+
  #stat_function(fun = line_mu,size = 0.8 )+
  geom_errorbar(aes (ymin = mu-mu_CI, ymax = mu+mu_CI, width = 0.3))+
...
AIC(sizefit_nlme_1,sizefit_nlme_2,sizefit_nlme_3, sizefit_nlme_4)


anova(sizefit_nlme_4)
...
anova(sizefit_nlme_1,sizefit_nlme_3)
...
#ylim(1,5)+
scale_x_discrete (limits = c("3.75", "10", "15", "20", "35", "50", "100", "500"))+   
  geom_errorbar(aes(ymin=x_2$down, ymax = x_2$up,width =0.4))+
  theme_bw()+
  xlab(expression (Fe[Total]~(nmol~L^-1)))+
...
#Sigma plot
 ggplot(photophys_fe, aes(x= Fee, y=sigma, color = Temperature))+
   stat_summary(size=1)+
   stat_summary(fun.data =  mean_se, geom = "errorbar",width = 0.05, aes(colour = Temperature ))+   
   scale_color_manual(values = c("black", "orange", "red"))+
   theme_bw()+
...
photophys_fe_fvfm_vs_sigma_2 <- dplyr::filter(photophys_fe_fvfm_vs_sigma, grepl("6.0", temperature))
...
X<- lm (fvfm ~ sigma*temperature, data = photophys_fe_fvfm_vs_sigma)
summary(X)
coef(X)
...
x<- lm(fvfm~Temperature*log(Fee), photophys_fe)
summary(x)
...
anova(fvfm_nlme_1, fvfm_nlme_4)

summary(fvfm_nlme_2)
#$tTable[,2]
      
newdat <- expand.grid(Temperature = c (1,3,6), Fee=7:1450)
newdat$wpred <- predict(fvfm_nlme_1,newdat,level=0)
