...
ggplot (dissociation_table, aes( x = pH, y = logKd, color = Temperature, shape = PAR))+
  ...
  theme (legend.title = element_text (size = 12), legend.text = element_text(size = 12), legend.position = c(0.85, 0.18))+
...
ggplot (dissociation_constants_pH, aes( x = Temperature, y = logKd, shape = PAR, group= PAR))+
  ...
  theme(axis.text.x=element_text(face = "bold", size = 12, color = "black"),
        ...
        axis.text.y=element_text(face = "bold", size = 12, color = "black"),
        axis.title.y=element_text(size=18,face="bold"))+
  theme (legend.title = element_text (size = 12), legend.text = element_text(size = 12), legend.position = c(0.85, 0.92))+
  labs (shape = expression(PAR~(mu*mol~photons~m^-2~s^-1)))+
 theme(panel.grid = element_blank())
...
#rfu_monod <- rfu [c(5,6,9:16, 19:25)]
#names(rfu_monod)<- c("culturetemp", "daycount", "3.75_A", "10_A", "15_A", "20_A","35_A", "50_A", "100_A", "500_A", "10_B", "15_B", "20_B","35_B", "50_B", "100_B", "500_B")
...
monod0.7C_rfu <- mergedata_sub[mergedata_sub$Temperature ==0.7,]
#monod0.7C_rfu <- subset(monod0.7C_rfu,  Fee<300)

monod1C_rfu <- mergedata_sub[mergedata_sub$Temperature ==1,]
#monod1C_rfu <- subset(monod1C_rfu,  Fee<300) 

monod2.7C_rfu <- mergedata_sub[mergedata_sub$Temperature ==2.7,]
#monod2.7C_rfu <- subset(monod2.7C_rfu,  Fee<300)

monod5.7C_rfu <- mergedata_sub[mergedata_sub$Temperature ==5.7,]
...
monod2.7C_rfu$Fee<- as.numeric (monod2.7C_rfu$Fee)
monod5.7C_rfu$Fee<- as.numeric (monod5.7C_rfu$Fee)


monod_0.7C_rfu <- nls(mumax ~ Vm * Fee/(K+Fee), data = monod0.7C_rfu, 
    start = list (K = 1E-5, Vm = max(monod0.7C_rfu$mumax)))

monod_1C_rfu <- nls(mumax ~ Vm * Fee/(K+Fee), data = monod1C_rfu, 
    start = list(K=1E-6, Vm = max(monod1C_rfu$mumax)))

monod_2.7C_rfu <- nls(mumax ~ Vm * Fee/(K+Fee), data = monod2.7C_rfu, 
    start = list(K=1E-6, Vm = max(monod2.7C_rfu$mumax)))

monod_5.7C_rfu <- nls(mumax ~ Vm * Fee/(K+Fee), data = monod5.7C_rfu, 
    start = list(K=1E-6, Vm = max(monod5.7C_rfu$mumax)))
...
monod_2.7 <- function(x){
  (0.27*x)/((31.83+x))
}

monod_5.7 <- function(x){
  (0.3458*x)/((28.275+x))
}
...
fit0.7C<-predict(as.lm.nls(monod_0.7C_rfu), interval = "confidence") 
monod0.7C_rfu$lowerfit<-fit0.7C[,2]
monod0.7C_rfu$upperfit<-fit0.7C[,3]

fit1C<-predict(as.lm.nls(monod_1C_rfu), interval = "confidence") 
monod1C_rfu$lowerfit<-fit1C[,2]
monod1C_rfu$upperfit<-fit1C[,3]

fit2.7C<-predict(as.lm.nls(monod_2.7C_rfu), interval = "confidence") 
monod2.7C_rfu$lowerfit<-fit2.7C[,2]
monod2.7C_rfu$upperfit<-fit2.7C[,3]


fit5.7C<-predict(as.lm.nls(monod_5.7C_rfu), interval = "confidence") 
monod5.7C_rfu$lowerfit<-fit5.7C[,2]
monod5.7C_rfu$upperfit<-fit5.7C[,3]

#mergedata_sub_1 <- mergedata_sub [-c(92,93),] 

#png("figure1_mu-vs-Fe.png", width=45*0.75, height=23*0.75, units="cm", res=900)
ggplot (mergedata_sub, aes (x = Fee, y = mumax, color = Temperature))+
  stat_summary(fun.y = mean, geom = "point",aes(group =Temperature, color = Temperature), size = 2.5)+
  stat_summary(fun.data =  mean_se, geom = "errorbar",width = 0.5, aes(colour = Temperature ))+
  scale_y_continuous(breaks = seq(0, 0.4, 0.05))+
  ylab (expression (Growth~rate~ (mu~d^-1)))+
  xlab (expression (Fe*"'"~ (pMole~L^-1)))+
  scale_color_manual(values = c("black", "white", "red", "darkred"))+
  theme(axis.text.x = element_text (angle = 60, hjust =1))+
  theme_bw()+
  stat_function(fun = monod_0.7, color = 'black')+
  stat_function(fun = monod_1, color = 'white')+
  stat_function(fun = monod_2.7, color = 'red')+
  stat_function(fun = monod_5.7, color = 'darkred')+
  theme(axis.text.x=element_text(face = "bold", size = 12, color = "black"),
        axis.title.x=element_text(size=18,face="bold"), 
        axis.text.y=element_text(face = "bold", size = 12, color = "black"),
        axis.title.y=element_text(size=18,face="bold"))+
...
# geom_ribbon(data=monod0.7C_rfu,
  ...
  #geom_ribbon(data=monod2.7C_rfu,
            #  aes(x = Fee, ymin=lowerfit, ymax=upperfit), alpha = 0.3, fill = "red", color = NA)+
 # geom_ribbon(data=monod5.7C_rfu, color = NA,
           #  aes(x = Fee, ymin=lowerfit, ymax=upperfit), alpha = 0.3, fill = "darkred", color = NA)+
...
Kfe <- c(28.275, 31.83, 63.11, 66.184)
Kfe_CI <- c(2.861, 3.196, 15.923, 11.966)
Temperature <- c(5.7, 2.7, 1, 0.7)
mu <- c(0.346, 0.271, 0.25, 0.177)
mu_CI <- c(0.01, 0.007, 0.02, 0.01)
...
ggplot (Km_temp, aes(x = Temperature, y = Kfe))+
  geom_point(size=5)+
  stat_function(fun = line_km, color = 'black', size = 0.8)+
  stat_function(fun = line_mu, color = "red", size = 0.8)+
  geom_point(aes( y = mu*150, color = "red", size = 5))+
  geom_errorbar(aes(ymin = mu*150 - mu_CI*150, ymax = mu*150 +mu_CI*150, color = "red"))+
  geom_errorbar(aes (ymin = Kfe-Kfe_CI, ymax = Kfe+Kfe_CI))+   
  scale_y_continuous (sec.axis = sec_axis( ~./150, name = expression(Maximum~growth~rate~(mu[max]~d^-1))))+
  theme(axis.text.y.right = element_text(angle = 45, hjust=9))+
  theme_bw()+
  ...
  xlab(expression (Temperature ~(degree*C)))+
  ylab (expression(K[Fe]~(pMole~Fe~ L^-1)))+
...
cellsize_growth <- merge(cellsize_sub, mergedata, by = "treatment")
...
png("figure3_size_vs_Fe.png", width=45*0.75, height=23*0.75, units="cm", res=900)
...
xlab (expression (Fe[Total]~ (nMole~L^-1)))+
  ...
  theme(panel.grid = element_blank())+
  labs(size = 12, col="Temperature (°C)")
dev.off()


#As temperature increases, cell size decreases because of increased division and growth. This causes cells to be more eficcience at taking nutrients, and reduces the nutrient quota
#This is to plot both Kfe and cell size on one plot to show that Kfe decreases with temperature 
# Km_temp$Kfe <- as.numeric(Km_temp$Kfe)
# merged <- merge (Km_temp, cellsize_growth)
# 
# ggplot(merged, aes(x=Temperature, y=adjustedsize))+
#   geom_point(aes(y = Kfe/10, color = "red"))+
#  scale_y_continuous (sec.axis = sec_axis( ~.*10, name = "Kfe (pM)"))+
#  geom_errorbar(aes(ymin =Kfe/10-Kfe_CI/10, ymax = Kfe/10+Kfe_CI/10, color = "red"))+
...
## his plots shows the change in size with temperature for the various iron concentrations. 
...
#x_2$names <- as.numeric(x_2$names)
...
ggplot (x_2, aes(x = names, y = Temperature))+ 
  ...
  theme_bw()+
  theme(panel.grid = element_blank())+
  scale_x_discrete (limits = c("10", "15", "20", "35", "50", "100", "500"))+
...
X<- ggplot(cellsize_growth_3, aes(x=Temperature, y=adjustedsize, color = Temperature))+
  geom_point(size = 3)+
  scale_color_gradient(high = "red", low = "blue")+
  theme_bw()+
  stat_summary(size = 5)+
...
x<- coef(lmList(adjustedsize~Temperature|totaliron_1 , data = cellsize_growth_3))
x_2 <- data.frame(x)
x_2 <- data.frame(names = row.names(x), x_2)
x_2$names <- as.numeric (as.character(x_2$names))
...
x<- c(10, 15, 20, 35, 50, 100, 500)
y <- c(4.4, 4.7, 4.6, 4.6, 3.5, 2.9, 1.75)
data <- data.frame(x,y)

png("figure4_reduction-insize_vs_Fe.png", width=45*0.75, height=23*0.75, units="cm", res=900)
ggplot (data, aes(x = x, y = y))+
  geom_point(size = 3)+
  scale_x_log10()+
  theme_bw()+
  xlab(expression (Fe[Total]~(nMole~L^-1)))+
  ylab(expression ('%'~Reduction~"in"~ECD~per~1*degree*C))+
  theme(axis.text.x=element_text(face = "bold", size = 12, color = "black"),
        axis.title.x=element_text(size=18,face="bold"), 
        axis.text.y=element_text(face = "bold", size = 12, color = "black"),
        axis.title.y=element_text(size=18,face="bold"))+
...
fvfm_fe_1 <- merge(fvfm_fe, mergedata_sub, by = "treatment")
 ...
 ggplot(fvfm_fe_1, aes(x= Fee, y=fvfm, color = Temperature))+
  ...
  stat_summary(size=1)+
   ...
   #scale_x_discrete (limits = c("0", "3.75", "10", "15", "20", "35", "50", "100", "500"))+
    theme_bw()+
    labs(col="Temperature (°C)")+
...
xlab (expression ("Fe'"~(pMole~L^-1)))+
  ...
  scale_x_log10()+
   scale_y_continuous(limits = c(0,0.6),breaks = seq(0,6, by = 0.1))
...
sigma_fe_1 <- merge(sigma_fe, mergedata_sub, by = "treatment")


 ggplot(sigma_fe_1, aes(x= treatment1, y=sigma, color = treatment2))+
  geom_point(size = 3)+
  xlab ("Fe'(pM)")+
 ylab ("Fv/Fm")+
 # stat_summary()+
  # scale_color_manual(values = c("black", "orange", "red"))+
   #scale_x_discrete (limits = c( "20","50", "100"))+
    theme_bw()+
    labs(col="Temperature (°C)")+
  ylab(expression (Absorption~cross~section))+
xlab (expression ("Fe'"~(pMole~L^-1)))+
  theme_bw()+
  theme(panel.grid = element_blank())+
  theme(axis.text.x=element_text(face = "bold", size = 12, color = "black"),
 ...
 x <- subset(sigma_fe_1, treatment2 == 10| treatment2==15|treatment2==20)
 
 
  
 ggplot(sigma_fe_1, aes(x= treatment2, y=sigma, color = culturetemp))+
  geom_point(size = 4)+
  scale_color_manual(values = c("black", "orange", "red"))+
   scale_x_discrete (limits = c("10","15","20","50", "100","500"))+
   #scale_x_discrete (limits = c( "10","500", "15", "20", "100"))+
    #scale_color_continuous()+
    #labs(col="Temperature (°C)")+
  #ylab(expression (Absorption~cross~section))+
#xlab (expression ("Fe'"~(pMole~L^-1)))+
...
x$Temperature<- as.numeric(as.character(x$Temperature))
sigma_fe_1$Temperature<- as.numeric(as.character(sigma_fe_1$Temperature))

z <- lm (sigma ~Temperature*treatment2, data = sigma_fe_1)  
summary(z)
